...
public class AccountService : IAccountService
{
  ...
  {
    // TODO: Single may not be the best to use here? Keeps throwing errors if multiple or none found
    Account? account = _context?.Account
      ?.Single(e => e.ApplicationUserId.Equals(userId));

    // TODO: Should we just except the default InvalidOperationException thrown? Or should we throw our own?
    if (account == null)
    {
